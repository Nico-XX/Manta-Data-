...
#data['planktonh'] = data['Plankton']
#data['planktonh'].replace({1:0, 2:0, 3:5, 4:5, 5:5}, inplace=True)
#data.head()
...
data['high'] = data["High tide"].fillna(0)
data['Mantas'] = data['Mantas'].fillna(0)
data['plankton5'] = data['plankton5'].fillna(0)
data['sunset'] = data['sunset'].fillna(0)
data['low'] = data['low'].fillna(0)
data['nboat'] = data['nboat'].fillna(0)
data['yboat'] = data['yboat'].fillna(0)
...
x_train, x_test, y_train, y_test = train_test_split(data[["nboat", "plankton5", "plankton4","plankton3","high"]], data[["Mantas"]], test_size=0.2, shuffle=True, random_state=1)
...
pois_reg = smf.poisson("Mantas ~ plankton5 + plankton4 + plankton3 + nboat + high", data=train2).fit()
